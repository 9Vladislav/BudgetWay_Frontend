...
export function EditTripForm({ trip, onBack, onSave }) {
  ...
  const originalTotal = Number(trip.totalCost) || 0;

  const [formData, setFormData] = useState({
    title: trip.title || trip.name || "",
    distance: String(trip.distanceKm || ""),
    fuelPrice: String(trip.fuelPrice || ""),
    cityPercent: trip.cityPercentage || trip.cityPercent || 50,
    food: trip.expenses?.food || "",
    tollRoads: trip.expenses?.tollRoads || "",
    accommodation: trip.expenses?.accommodation || "",
  ...
  useEffect(() => {
    const loadCars = async () => {
      try {
        ...
        const currentCar =
          loadedCars.find((car) => car.id === trip.carId) || loadedCars[0];
  ...
  const priceDifference = calculations.totalCost - originalTotal;
      ...
      <div className="mx-auto max-w-330">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-[16px] text-slate-600">
            Оновіть дані поїздки — витрати перераховуються в реальному часі
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex items-start gap-6">
          <div className="flex w-full max-w-205 flex-col gap-6">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white px-6 py-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                <ExpenseInput label="Їжа (₴)" name="food" value={formData.food} onChange={handleChange} />
                <ExpenseInput label="Платні дороги (₴)" name="tollRoads" value={formData.tollRoads} onChange={handleChange} />
                <ExpenseInput label="Проживання (₴)" name="accommodation" value={formData.accommodation} onChange={handleChange} />
                <ExpenseInput label="Інше (₴)" name="other" value={formData.other} onChange={handleChange} />
              </div>
            </section>
          </div>
          ...
          <aside className="w-88 rounded-2xl border border-emerald-200 bg-emerald-50 px-6 py-6">
            ...
            <div className="mb-6 space-y-4 text-[14px] text-slate-600">
              ...
              {calculations.food > 0 && <SummaryRow label="Їжа" value={calculations.food} />}
              {calculations.tollRoads > 0 && <SummaryRow label="Платні дороги" value={calculations.tollRoads} />}
              {calculations.accommodation > 0 && <SummaryRow label="Проживання" value={calculations.accommodation} />}
              {calculations.other > 0 && <SummaryRow label="Інше" value={calculations.other} />}
            </div>
            ...
            <div className="mb-5 flex justify-between rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-[14px] font-bold text-blue-600">
              <span>
                {priceDifference > 0
                  ? "Вартість збільшилась"
                  : priceDifference < 0
                    ? "Вартість зменшилась"
